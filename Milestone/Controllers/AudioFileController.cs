...
namespace DotNet5Crud.Controllers
{
    ...
    */
    public class AudioFileController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> uploadComment(string comment, AudioFile audioFile)
        {

            SecurityDAO securityDAO = new SecurityDAO();

            if(securityDAO.uploadComment(comment, audioFile.AudioFileId))
            {
        ...
        public async Task<IActionResult> AddOrEdit2(IFormCollection formCollection, IFormFile file)
        {
            ...
            AudioFile audioFile = await _context.AudioFiles.FindAsync(3);
        ...
        [HttpPost]
        [DisableRequestSizeLimit]
        public async Task<IActionResult> AddOrEdit(IFormCollection formCollection, IFormFile file)
        {
            int userID1 = (int)HttpContext.Session.GetInt32("userID");
            ...
            try
            {
                ...
                else
                {
                    ...
                    Task NewMethod(ConfigWrapper config)
                    {
                        // might need to change back to just return  RunAsync(config, fileNameWithPath, OutputFolderName, audioFile);
                        return Task.Factory.StartNew(() => RunAsync(config, fileNameWithPath, OutputFolderName, audioFile));
        ...
        /// <returns></returns>
        // <RunAsync>
        private async Task RunAsync(ConfigWrapper config, string mp4name, string OutputFolderName, AudioFile audiofileData)
        {
            ...
            _ = await CreateInputAssetAsync(client, config.ResourceGroup, config.AccountName, inputAssetName, mp4name);

            // Use the name of the created input asset to create the job input.
            _ = new JobInputAsset(assetName: inputAssetName);

            // Output from the encoding Job must be written to an Asset, so let's create one
            Microsoft.Azure.Management.Media.Models.Asset outputAsset = await CreateOutputAssetAsync(client, config.ResourceGroup, config.AccountName, outputAssetName);
